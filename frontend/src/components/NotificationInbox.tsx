...
// Caixa de Entrada do Mural: notificações capturadas de outros apps,
// interpretadas e prontas pra virar missão/gasto. Era um overlay em tela
// cheia; agora é uma sub-aba normal (conteúdo sempre montado quando ativa).
  ...
  return (
    <div className="page">
      <div className="notif-inbox-header">
        <p className="hint notif-settings-hint" style={{ margin: 0 }}>
          Escolha quais apps monitorar (e priorize os bancos) em{" "}
          <button className="link-btn" onClick={() => navigate("/regras")}>
            Ajustes › Notificações monitoradas
          </button>
          .
        </p>
        <button className="icon-btn" onClick={() => navigate("/regras")} aria-label="Configurar apps monitorados">
      ...
      {!granted && (
        <div className="notif-access-card">
          <p>
            Ative o acesso a notificações em Ajustes do Android pra ver aqui as notificações reais, agrupadas por
            app.
          </p>
      ...
      <div className="notif-groups">
        ...
        {!loading && !hasMonitoredApp && (
          <p className="hint">
            Não há notificações recentes — ative pelo menos um app em Ajustes › Notificações monitoradas pra
            começar.
          </p>
        )}
